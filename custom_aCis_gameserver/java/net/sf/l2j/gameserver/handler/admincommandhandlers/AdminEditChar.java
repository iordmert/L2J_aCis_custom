	...
	public boolean useAdminCommand(String command, L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_rec"))
		{
			...
			try
			{
				...
				int pvpCount = Integer.parseInt(command.substring(13));
				player.setPvpKills(pvpCount);
				player.sendMessage("You have been given pvps by a GM["+ activeChar.getName()  +"]");
				player.updatePvPColor();
				player.broadcastUserInfo();
			}
			catch (Exception e)
			{
				activeChar.sendMessage("Usage: //setpvp number");
			}
